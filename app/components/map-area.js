...
export default Ember.Component.extend({
  ...
  addMarker: function (location, lat, lon) {
  ...
  tryNominatim: function (location) {
    let locationParam = location.replace(/\s+/g, " ").trim().split(" ").join("+");
    Em.$.getJSON(this.get('nominatimUrl') + `?format=json&q=${locationParam}`).then((data) => {
      if (data[0] && data[0].lat && data[0].lon) {
        this.addMarker(location, data[0].lat, data[0].lon);
      }
    });
  },

  tryGoogle: function (location, lat, lon) {
    let geocoder = new google.maps.Geocoder();
    geocoder.geocode({'address': location}, (results, status) => {
      if (status === google.maps.GeocoderStatus.OK) {
        let coords = results[0].geometry.location;
        this.addMarker(location, coords.H, coords.L);
      }
      else if (status == google.maps.GeocoderStatus.OVER_QUERY_LIMIT) {
        console.log('google over limit tryin nominatim...');
